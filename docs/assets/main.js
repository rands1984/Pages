(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const i of document.querySelectorAll('link[rel="modulepreload"]'))r(i);new MutationObserver(i=>{for(const o of i)if(o.type==="childList")for(const n of o.addedNodes)n.tagName==="LINK"&&n.rel==="modulepreload"&&r(n)}).observe(document,{childList:!0,subtree:!0});function s(i){const o={};return i.integrity&&(o.integrity=i.integrity),i.referrerPolicy&&(o.referrerPolicy=i.referrerPolicy),i.crossOrigin==="use-credentials"?o.credentials="include":i.crossOrigin==="anonymous"?o.credentials="omit":o.credentials="same-origin",o}function r(i){if(i.ep)return;i.ep=!0;const o=s(i);fetch(i.href,o)}})();var l="transcription_gas_api_url";function d(){const t=localStorage.getItem(l);return t||document.querySelector('meta[name="gas-api-url"]')?.content||"https://script.google.com/macros/s/AKfycbxVN3iTnaUBpeuUBGLwAp7vIthoscUWeZ65hq8whzorai7ae6qn1EEh16BY4fmg8WYhuA/exec"}var c=d(),u=class{async post(t,e={}){let s;try{s=await fetch(c,{method:"POST",headers:{"Content-Type":"text/plain;charset=utf-8"},body:JSON.stringify({action:t,...e})})}catch(o){throw new Error("ネットワーク/CORSエラー: "+o.message+" (URL: "+c+")")}const r=await s.text();let i;try{i=JSON.parse(r)}catch{const o=r.replace(/\s+/g," ").slice(0,150);throw new Error("GASがJSON以外を返却(URL無効/削除/未デプロイ?): "+o)}if(i.status==="error")throw new Error(i.error||"GAS error");return i}async getSttToken(t){return(await this.post("stt_token",{provider:t})).token}async summarize(t,e){return(await this.post("summarize",{transcript:t,interval:e})).summary}async save(t,e,s,r){return await this.post("save",{transcript:t,mode:e,sttProvider:s,summaryInterval:r})}},h=class{ws=null;provider;token;onTranscript;onError;onClose;onOpen;reconnectAttempts=0;maxReconnect=5;sendQueue=[];closedByUser=!1;sentBytes=0;onDebug;constructor(t,e,s){this.provider=t,this.token=e,this.onTranscript=s.onTranscript,this.onError=s.onError,this.onClose=s.onClose,this.onOpen=s.onOpen,this.onDebug=s.onDebug||(()=>{})}connect(){this.closedByUser=!1,this.open()}buildUrl(){return this.provider==="speechmatics"?`wss://eu2.rt.speechmatics.com/v2?jwt=${encodeURIComponent(this.token)}`:`wss://api.deepgram.com/v1/listen?${new URLSearchParams({model:"nova-2",language:"ja",smart_format:"true",interim_results:"true",punctuate:"true",diarize:"true"}).toString()}`}open(){const t=this.buildUrl();try{this.provider==="deepgram"?this.ws=new WebSocket(t,["token",this.token]):this.ws=new WebSocket(t)}catch(e){this.onError(e instanceof Error?e:new Error(String(e)));return}this.ws.binaryType="arraybuffer",this.ws.onopen=()=>{for(this.reconnectAttempts=0,this.onOpen(),this.onDebug("ws","OPEN");this.sendQueue.length>0;){const e=this.sendQueue.shift();this.ws.send(e)}},this.ws.onmessage=e=>{this.onDebug("msg",(typeof e.data=="string"?e.data:"[binary]").slice(0,200)),this.handleMessage(e.data)},this.ws.onerror=()=>{this.onError(new Error(`WebSocket error: ${this.provider}`))},this.ws.onclose=e=>{if(this.closedByUser){this.onClose(e.code);return}this.reconnectAttempts<this.maxReconnect?(this.reconnectAttempts++,setTimeout(()=>this.open(),1e3*this.reconnectAttempts)):this.onClose(e.code)}}handleMessage(t){if(this.provider==="speechmatics"){let e;try{e=JSON.parse(t)}catch{return}if(e.message==="AddTranscript"&&e.results?.length>0)for(const s of e.results){const r=(s.alternatives?.[0]?.speaker??0)===0?a.micLabel:a.cableLabel;this.onTranscript({time:this.nowStr(),speaker:r,text:s.alternatives?.[0]?.content||"",final:!0})}}else{let e;try{e=JSON.parse(t)}catch{return}if(e.type==="Results"&&e.channel?.alternatives?.[0]){const s=e.channel.alternatives[0],r=(s.words?.[0]?.speaker??0)===0?a.micLabel:a.cableLabel;this.onTranscript({time:this.nowStr(),speaker:r,text:s.transcript||"",final:e.is_final===!0})}}}sendAudio(t){this.sentBytes+=t.byteLength,this.onDebug("audio",this.sentBytes+" bytes"),this.ws?.readyState===WebSocket.OPEN?this.ws.send(t):this.ws?.readyState===WebSocket.CONNECTING&&this.sendQueue.push(t)}close(){this.closedByUser=!0,this.ws&&(this.ws.onclose=null,this.ws.close(),this.ws=null)}nowStr(){const t=new Date,e=s=>s<10?"0"+s:""+s;return`${e(t.getMinutes())}:${e(t.getSeconds())}`}},a={micLabel:"自分",cableLabel:"相手"},p=class{audioContext=null;source=null;processor=null;stream=null;stt=null;sampleRate=16e3;onError;onRms=null;constructor(t,e){this.onError=t,this.onRms=e||null}async start(t,e){this.stt=t;const s={audio:e?{deviceId:{exact:e},echoCancellation:!1,noiseSuppression:!1,autoGainControl:!1}:{echoCancellation:!1,noiseSuppression:!1,autoGainControl:!1}};this.stream=await navigator.mediaDevices.getUserMedia(s);const r=window.AudioContext||window.webkitAudioContext;this.audioContext=new r({sampleRate:this.sampleRate}),this.audioContext.state==="suspended"&&await this.audioContext.resume(),this.source=this.audioContext.createMediaStreamSource(this.stream);const i=4096;this.processor=this.audioContext.createScriptProcessor(i,1,1),this.processor.onaudioprocess=o=>this.onAudioProcess(o),this.source.connect(this.processor),this.processor.connect(this.audioContext.destination)}onAudioProcess(t){if(!this.stt)return;const e=t.inputBuffer.getChannelData(0),s=new Int16Array(e.length);let r=0;for(let o=0;o<e.length;o++){const n=Math.max(-1,Math.min(1,e[o]));s[o]=n<0?n*32768:n*32767,r+=n*n}const i=Math.sqrt(r/e.length);this.onRms&&this.onRms(i),this.stt.sendAudio(s.buffer)}stop(){this.processor&&(this.processor.disconnect(),this.processor=null),this.source&&(this.source.disconnect(),this.source=null),this.stream&&(this.stream.getTracks().forEach(t=>t.stop()),this.stream=null),this.audioContext&&(this.audioContext.close(),this.audioContext=null)}},m=class{gas=new u;audio=new p(t=>this.log("音声エラー: "+t.message),t=>this.updateDebug("rms",t.toFixed(4)));stt=null;transcripts=[];recording=!1;mode="inperson";sttProvider="deepgram";summaryInterval=3;lastSummary="";summaryTimer=null;elStatus;elTranscript;elSummary;elMicSelect;async init(){this.buildUI(),await this.loadDevices()}buildUI(){document.body.innerHTML=`
      ...
      <div class="settings">
        ...
        <input id="inpGasUrl" type="text" placeholder="https://script.google.com/macros/s/.../exec" value="${c}">
