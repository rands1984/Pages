...
function doPost(e) {
  try {
    ...
    if (missing) {
      return createCorsResponse(createErrorResponse('Script property not configured', missing));
    }

    var postData = e.postData?.contents;
    if (!postData) {
      return createCorsResponse(createErrorResponse('Empty request body'));
    }
    ...
    try {
      ...
      return createCorsResponse(createErrorResponse('Invalid JSON', parseErr.message));
    }

    if (request.action === 'summarize') {
      return createCorsResponse(handleSummarize(request));
    } else if (request.action === 'save') {
      return createCorsResponse(handleSave(request));
    } else if (request.action === 'stt_token') {
      return createCorsResponse(handleSttToken(request));
    } else {
      return createCorsResponse(createErrorResponse('Unknown action', request.action));
    }
  } catch (err) {
    var message = err instanceof Error ? err.message : String(err);
    return createCorsResponse(createErrorResponse('Internal server error', message));
  }
}
